...
function normalizeDates(user, formFields) {
  const result = { ...user };

  // Ajoutez toujours le champ role, même si vide
  result.role = user.roles?.length > 0 ? user.roles[0].id : null;
...
const form = useForm(
  normalizeDates(props.user, props.form)
)

function submitForm() {
  form.put(props.resource.routes.update.replace(':id', props.user.id), {
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <template v-for="(field, key) in props.form" :key="key">
            <div v-if="field.type === 'select'" class="space-y-2">
              ...
              <Select v-model="form.role" :required="field.options?.required">
