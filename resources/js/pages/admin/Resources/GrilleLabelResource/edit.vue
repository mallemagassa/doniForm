...
function normalizeDates(grillelabel, formFields) {
  const result = { ...grillelabel }
...
const form = useForm(
  normalizeDates(props.grillelabel, props.form)
)

function submitForm() {
  form.put(props.resource.routes.update.replace(':id', props.grillelabel.id), {
